...
""" 
    ...
    def auto_play(self, max_turns=200, verbose=True):
        turn = 0
        while self.game.phase != GamePhase.GAME_OVER and turn < max_turns:
            actions = self.game.get_actions()
            if not actions:
                break
            game_state = extract_game_state(self.game)
            chosen_action = self.game.currentController.chooseAction(actions, game_state)
            self.game.process_action(chosen_action)
            turn += 1
            if verbose:
                print(f"Turn {turn}, Action: {chosen_action}")
