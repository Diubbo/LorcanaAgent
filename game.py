...
class Game:
        ...
        try:
            if self.phase == GamePhase.MULLIGAN:
                ...
                if isinstance(act, PassAction):
                    # no valid targets → cancel challenge
    ...
    def get_actions(self):
        if self.phase == GamePhase.MULLIGAN:
            ...
            actions = self.currentOpponent.get_challenge_targets(self, not self.current_challenger.evasive)
            if not actions:
                return [PassAction()]
            return actions        
        elif self.phase == GamePhase.CHOOSE_TARGET:
