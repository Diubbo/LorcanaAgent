...
from ability import Ability, DamageTriggeredAbility,HealingTriggeredAbility,PascalGainEvasiveAbility,ReadyPrincessAbility
from ability import HealAllCharactersAbility, InkTopCardAbility, DamageAllOpponentsAbility, DrawCardsActionAbility,TargetedHealingAbility, BanishItemAbility
@dataclass
class Decklist:
    name: str
    cards: list
...
class Card:
    ...
    def requires_target(self) -> bool:
        for ab in self.abilities:
            if isinstance(ab, (HealingTriggeredAbility, DamageTriggeredAbility,
                               TargetedHealingAbility, BanishItemAbility)):
                return True
        return False
...
dinglehopper = ItemCard("32/204-EN-1","Dinglehopper", "Remove up to 1 damage from chosen character", (HealingTriggeredAbility(1,needs_to_banish=False),), 1, "amber", True, tuple())
...
maleficent = CharacterCard("49/204-EN-1", "Maleficent", "When you play this character you may draw a card.", tuple(), 3,"amethyst", True, ("Storyborn", "Villain", "Sorcerer"), "Sorceress", 
         2,2,1,tuple())
friends_on_the_other_side = ActionCard("64/204-EN-1", "Friends On The Other Side", "Draw 2 cards", (DrawCardsActionAbility(cards_to_draw=2),), 3,"amethyst", True, ("Song",))
...
hakuna_matata=ActionCard("27/204-EN-1", "Hakuna Matata","Remove up to 3 damage from each of your characters.", (HealAllCharactersAbility(healing_power=3),), 4, "amber", True, ("Song",))
...
healing_glow = ActionCard("28/204-EN-1", "Healing Glow", "Remove up to 2 damage from chosen character.", (TargetedHealingAbility(2),), 1, "amber", True, ())
pumba = CharacterCard("17/204-EN-1", "Pumba", "", tuple(), 4,"amber", True, ("Storyborn", "Ally"), "Friendly Warhog", 3,5,1,tuple())
...
amber_amethyst = Decklist("Amber/Amethyst",[
    ...
    heihei,heihei,mickey_mouse_true_friend, mickey_mouse_true_friend,
    mickey_mouse_true_friend,minnie_mouse,minnie_mouse,minnie_mouse,
    healing_glow,healing_glow,healing_glow, yzma,yzma,ariel,ariel,
    ...
    pumba,pumba,rafiki,rafiki,rafiki,sven,flotsam,jetsam,jetsam,
...
one_jump_ahead = ActionCard("164/204-EN-1", "One Jump Ahead","Put the top card of your deck into your inkwell facedown and exterted", (InkTopCardAbility(),), 2,"sapphire",False, ("Song",))
...
fire_the_cannons = ActionCard("197/204-EN-1", "Fire the Cannon!",  "Deal 2 damage to chosen character", (DamageTriggeredAbility(2),), 1,"steel",False, tuple())
...
smash= ActionCard("200/204-EN-1", "Smash", "Deal 3 damage to a chosen character.", (DamageTriggeredAbility(3),), 3, "steel", True, tuple())
...
grab_your_sword = ActionCard("198/204-EN-1", "Grab Your Sword", "Deal 2 damage to each opposing character", (DamageAllOpponentsAbility(damage_power=2),), 5, "steel", False, ("Song",))
...
magic_golden_flower = ItemCard("169/204-EN-1", "Magic Golden Flower", "Banish this item - remove up to 3 damage from chosen character",
            (HealingTriggeredAbility(3,needs_to_banish=True),),  1, "sapphire", True, tuple())
...
lilo = CharacterCard("184/204-EN-1", "Lilo", "Space. That's where aliens come from. And also tourists!", tuple(), 3,"steel", True, ("Dreamborn", "Hero"), "Galactic Hero", 4,2,2,tuple())
break2 = ActionCard("196/204-EN-1", "Break", "Banish Chosen Item.", (BanishItemAbility(),), 2,"steel", True, tuple())
belle = CharacterCard("142/204-EN-1", "Belle", "", tuple(), 4,"sapphire", True, ("Storyborn", "Hero", "Princess"), "Strange but Special", 2,4,1,tuple())
sapphire_steel = Decklist("Sapphire/Steel", [
    captain_hook,captain_hook,captain_hook,aurora,aurora,
    maleficent,maleficent,maleficent,simba,
    scar_blue,one_jump_ahead,one_jump_ahead,kristoff,
    kristoff,one_jump_ahead,flounder,flounder,
